...
class Controlador(app_manager.RyuApp):
    ...
    def port_status_handler(self, ev):
        ...
        #
        #     if self.topo_shape.switch_ports_state[dpid]['puertos'][port.port_no] == port.state:
        #         # Do nothing
        ...
        #     if port_down:
        #         print('LINK DOWN')
        #         print('estado de puerto {} es {}'.format(port.port_no, port.state))
        #     else:
        #         if port.state == 0:
        #             print('estado de puerto {} es {}'.format(port.port_no, port.state))
        #         if port.state == 4:
        ...
        if reason is dp.ofproto.OFPPR_ADD:
            ...
            if port_down and self.topo_shape.switch_ports_state[dpid]['puertos'][port.port_no] == 'UP':
                ...
                'HAY QUE CREAR EN EL JSON DE LA TOPOLOGIA LOS LINKS DICIENDO QUE SWITCHES CONECTAN Y EN QUE PUERTOS'
                'CADA VEZ QUE SE CAE UN PUERTO HAY QUE VERIFICAR EN LOS LINKS DE LA TOPOLOGIA, SI LOS PUERTOS QUE UNEN'
                'DOS SWITCHES ESTAN DOWN, SI ESO PASA SE DA DE BAJA EL PUERTO QUE CONECTA AMBOS SWITCHES Y SE INFORMA QUE EL LINK'
    ...
    def agregar_conexion_entre_switches(self, dpid_1, dpid_2, puerto1, puerto2):
        switch_1 = self.obtener_nombre_switch(dpid_1)
        switch_2 = self.obtener_nombre_switch(dpid_2)
        print('switch 1 : {}'.format(switch_1))
        print('switch 2 : {}'.format(switch_2))

        self.conexion_switches[switch_1][switch_2] = puerto1
        self.conexion_switches[switch_2][switch_1] = puerto2
        #print(self.conexion_switches)
...
class TopoStructure():
    ...
    def find_links_with_src(self, dpid):
        d_links = []
        for l in self.topo_raw_links:
            if l.src.dpid == dpid:
                d_links.append(l)
    ...
    def obtener_link_de_lista_down(self, dpid, port):
        retorno = None
