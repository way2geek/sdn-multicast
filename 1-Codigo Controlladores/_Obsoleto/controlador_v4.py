...
class Controlador(app_manager.RyuApp):
    ...
    def creo_group_table_en_switches(self, group_id, IPV4dst, switch_origen):
        switches_del_camino = {}
        puertos = {}
        self.puertos_salida_switches_camino.setdefault(group_id, {})
        for switch_destino in self.camino_grupos[group_id]['switches_destino']:
            ...
            for switch_camino in switches_del_camino[switch_destino]:
                datapath = self.datapath_switch[switch_camino]['datapath']
                puertos.setdefault(switch_camino, [])

                if self.existe_flujo_group_table(switch_camino, group_id):

                    self.puertos_salida_switches_camino[group_id][switch_destino].update({switch_camino:self.genero_puertos_salida(datapath, switch_origen, switch_camino, switch_destino, group_id)})

                    print('LOS PUERTOS DE SALIDA DE LOS SWITCHES DEL GRUPO {} SON {}'.format(group_id, self.puertos_salida_switches_camino))
                    puertos[switch_camino].extend(self.puertos_salida_switches_camino[group_id][switch_destino][switch_camino])
                    print('LOS PUERTOS SON {}'.format(puertos))
                    #self.encamino_trafico_multicast(datapath, group_id, self.puertos_salida_switches_camino, switch_destino, switch_camino)
    ...
    def genero_puertos_salida(self, datapath, switch_origen, switch_camino, switch_destino, group_id):
        ...
        if switch_camino != switch_destino :
            ...
            puertos_de_salida.append(self.caminos_completos[switch_origen][switch_destino][switch_camino])
