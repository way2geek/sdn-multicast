...
class Controlador(app_manager.RyuApp):
    ...
    def switch_features_handler(self, ev):
        ...
        # app_simplificado_v3.load_json_topology()
        #
        # self.conexion_switches = app_simplificado_v3.switches
        # self.conexion_hosts_switches = app_simplificado_v3.hosts
        # self.dpids = app_simplificado_v3.dpids
        #
        # app_simplificado_v3.shortest_paths_all()
        #
        ...
        if not req_igmp:
            ...
            if self.esMulticast(destino):
                ...
                if self.es_origen(datapath, in_port):
                    ...
                    #print('SWITCH {} ES ORIGEN'.format(switch_origen))
                    #print(IPV4dst)
                    ...
                    if IPV4dst in self.lista_grupos:
                        ...
                        #self.encamino_trafico_multicast(datapath, group_id)
    ...
    #
    #     self.caminos_entre_hosts = app_simplificado_v3.camino_entre_hosts()
    #     self.caminos_completos = app_simplificado_v3.caminos_completos()
    #     #print('LOS CAMINOS COMPLETOS SON {}'.format(self.caminos_completos))
    ...
    # def encamino_trafico_multicast(self, datapath, group_id, origen):
    #
    #     switch_origen = origen
    #     puertos_destino_1 = []
    #     puertos_destino_2 = []
    #
    #     for switch_destino in self.camino_grupos[group_id]['switches_destino']:
    #         if self.camino_entre_switches[origen][switch_destino] == None:
    #             'ESTOY EN EL MISMO SWITCH'
    #             puertos_destino_1 = camino_grupos[group_id]['switches_destino'][switch_destino]
    #             buckets = self.generoBuckets(datapath, puertos_destino)
    #
    #         else:
    #             'DEBO OBTENER PUERTOS DE SALIDA DE CADA SWITCH EN EL CAMINO AL DESTINO PARA EL GRUPO DETERMINADO'
    #             'EN CADA SWITCH EN EL CAMINO DEBO MODIFICAR LA GROUP TABLE CON EL PUERTO DE SALIDA'
    #             #recorro todos los switches y en cada uno con su datapath modifico la group table
    ...
    def creo_group_table_en_switches(self, group_id, IPV4dst, switch_origen):
        ...
        #print(self.camino_grupos)
        for switch_destino in self.camino_grupos[group_id]['switches_destino']:
            switches_del_camino.setdefault(switch_destino, [])
            switches_del_camino[switch_destino] = self.caminos_completos[switch_origen][switch_destino].keys()
            print('Los switches del camino de origen {} y destino {} son {}'.format(switch_origen, switch_destino, switches_del_camino[switch_destino]))

            for switch in switches_del_camino[switch_destino]:
                datapath = self.datapath_switch[switch]['datapath']
                if self.existe_flujo_group_table(switch, group_id):

                    print('YA SE ENCUENTRA REGISTRADO EL FLUJO AL GRUPO {} EN EL SWITCH {}'.format(group_id, datapath.id))
                    'SE DEBE MODIFICAR LA GROUP TABLE'
                    'FALTA AGREGAR FUNCION PARA OBTENER LA COOKIE CORRESPONDIENDTE AL GRUPO'
                    ...
                    self.datapath_switch[switch]['cookies_grupos'].append(group_id)
    ...
    def add_flow(self, datapath, table_id, priority, match, instructions, buffer_id=None, cookie=None):
