...
class Controlador(app_manager.RyuApp):
    ...
    def manejo_trafico_multicast(self, group_id, IPV4dst, switch_origen):
        ...
        for switch_destino in self.camino_grupos[group_id]['switches_destino']:
            ...
            self.camino_grupos[group_id]['camino'][switch_destino][switch_destino] = self.camino_grupos[group_id]['switches_destino'][switch_destino][0]
    ...
    def genero_puertos_salida(self, datapath, switch_camino, switch_destino, group_id):
        ...
        if switch_camino != switch_destino :
            ...
            if switch_camino in self.camino_grupos[group_id]['camino']:
                array_aux = self.camino_grupos[group_id]['switches_destino'][switch_destino]
                puertos_de_salida.extend(array_aux)
                print('SWITCH CAMINO {} DIFERENTE DESTINO {} CON PUERTOS DE SALIDA {}'.format(switch_camino, switch_destino, puertos_de_salida))

            for switch_aux in self.camino_grupos[group_id]['camino']:
                if switch_camino in self.camino_grupos[group_id]['camino'][switch_aux]:
                    puerto = self.camino_grupos[group_id]['camino'][switch_aux][switch_camino]
                    if puerto not in puertos_de_salida:
                        puertos_de_salida.append(self.camino_grupos[group_id]['camino'][switch_aux][switch_camino])
                    print('SWITCH CAMINO {} DIFERENTE DESTINO {} CON PUERTOS DE SALIDA {}'.format(switch_camino, switch_aux, puertos_de_salida))

        elif switch_camino == switch_destino:
            array_aux = self.camino_grupos[group_id]['switches_destino'][switch_destino]
            puertos_de_salida.extend(array_aux)
            print('SWITCH CAMINO {} ES EL DESTINO {} CON PUERTOS DE SALIDA {}'.format(switch_camino, switch_destino, puertos_de_salida))


        # elif switch_camino in self.camino_grupos[group_id]['camino'] and switch_camino in self.camino_grupos[group_id]['camino'][switch_destino]:
        #
        #     puertos_de_salida.append(self.camino_grupos[group_id]['camino'][switch_destino][switch_camino])
        #
        #     array_aux = self.camino_grupos[group_id]['switches_destino'][switch_destino]
